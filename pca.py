...
PR = pd.read_csv(rootDir + r'/ressources/PR_csv_tmp', low_memory=False)
...
main_analysis = PR[["HV106", "HV108","HV109","HV121","HV122","HV124"]]
main_analysis.dropna(axis = 0, how = 'all', inplace = True)

for col in ["HV106", "HV108","HV109","HV121","HV122","HV124"]:
    main_analysis[col].fillna((main_analysis[col].mean()), inplace=True)
...
### Main_analysis_merge
main_analysis_merge = main_analysis_merge.sort_values(by=['COEF'], ascending=False).round(2)
pd.DataFrame(main_analysis_merge).to_csv(rootDir + "/QA.csv")
print(main_analysis_merge)
...
pca_6.fit(X_scaled)
X_pca_6 = pca_6.transform(X_scaled)

print("Variance explained by all 6 principal components =", sum(pca_6.explained_variance_ratio_ * 100))
...
plt.savefig(rootDir + "/Variance_Explained_6.png", dpi=100)
...
print("Variance explained by all 3 principal components =", sum(pca_3.explained_variance_ratio_ * 100))
...
plt.xlabel("Number of components")
plt.ylabel("Explained variance")
plt.savefig(rootDir + "/Variance_Explained_3.png", dpi=100)
...
plt.xlabel("Number of components")
plt.ylabel("Explained variance")
plt.savefig(rootDir + "/Variance_Explained_2.png", dpi=100)
...
PCAs = pd.DataFrame(X_pca_95, columns = ["PC1", "PC2", "PC3", "PC4"])
PCAs["CASE"] = PR["CASEID"]
print(PCAs.head())
