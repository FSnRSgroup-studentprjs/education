...
def load_dhs_data(dhs_path, return_paths=False):
    ...
    country_d = {}
    input_file = csv.reader(open(r"/mnt/datadisk/data/surveys/DHS_info/country_codes.csv"), delimiter='\t')
    for row in input_file:
        country_d[row[0]] = row[1]

    data_file_types_d = {}
    input_file = csv.reader(open(r"/mnt/datadisk/data/surveys/DHS_info/data_file_types.csv"), delimiter='\t')
    for row in input_file:
